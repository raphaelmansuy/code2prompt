...
def display_price_table(config: Configuration, token_count: int):
    token_prices = load_token_prices()
    if not token_prices:
        return

    price_results = calculate_prices(
        token_prices, token_count, config.output_tokens, config.provider, config.model
    )
    ...
    for result in price_results:
        ...
        specific_output_price = format_specific_price(result.price_output, config.output_tokens)
        ...
        table.add_row(
            ...
            input_price,
            output_price,
            f"{token_count:,} | {config.output_tokens:,}",
    ...
    console.print("💡 Tip: 'Price $ In | Out' shows the cost for the specific input and output tokens.")
    console.print("⚠️  This is an estimate based on OpenAI's Tokenizer implementation.\n")